...
def split_data(data,n,m):
    ...
    for i in range(n_samples):
        ...
        for k in range(0, cols):
            if k == 1:
                continue;
            input_data_catboost.append(data[i+m+n-1,k])
            input_data.append(data[i+m+n-1,k])
...
def split_future_data(data,m):
    ...
    for j in range(m,len(data)):
        ...
        input_data_catboost.extend(data[j][[0,2,3,4,5,6]])
        in_catboost.append(input_data_catboost)
...
# in_catboost , out_single = split_future_data(data,m)
# in_catboost = np.array(in_catboost)
...
columns_names += ['time_672','is_weekday_672','weather_cold_672','weather_warm_672','weather_hot_672','month_672']
...
columns_names += [j+'_'+str(i) for i in range(0,n_steps+1) for j in ['value','is_weekday','weather_cold','weather_warm','weather_hot','month']]
#最后的六个数据被裁剪为5个(delete value_672)
columns_names.remove('value_672')
