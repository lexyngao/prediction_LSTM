...
def train_step(inp, tar):
    ...
    with tf.GradientTape() as tape:
        ...
        loss = loss_function(tf.reshape(tar, predictions.shape), predictions)
...
train_label = ss_Y.transform(train_label).reshape(-1,96)
test_label = ss_Y.transform(test_label).reshape(-1,96)
...
if train_again:
    ...
    for epoch_n in range(EPOCHS):
        ...
        for batch_n in range(num_valid_batches):
            ...
            predictions = tf.reshape(transformer(inp_tf, False, enc_padding_mask), [-1, 96])
...
plt.figure()
plt.plot(truth[0,:], c="r", label='real')
plt.plot(predict[0,:], c="b", label='pred')
# plt.plot(truth[:, -1], c="r", label='real')
plt.grid()
plt.legend()
plt.title('test set')
